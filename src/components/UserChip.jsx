...
export default function UserChip({ favoriteCount = 0, onOpenAuth, onOpenFavorites }) {
  ...
  if (auth.status === 'loading' || !auth.isProvisioned) return null;
    ...
    <div className="user-chip-wrap" ref={rootRef}>
      ...
      {open && (
        <div className="user-menu" role="menu">
          ...
          <button type="button" role="menuitem" onClick={() => { setOpen(false); onOpenAuth?.('profile'); }}>
            <FiUser aria-hidden="true" /> Profil et sécurité
          </button>
          <button type="button" role="menuitem" onClick={() => { setOpen(false); onOpenFavorites?.(); }}>
            <FiHeart aria-hidden="true" /> Mes favoris{favoriteCount ? ` (${favoriteCount})` : ''}
          </button>
          <button type="button" role="menuitem" onClick={async () => { setOpen(false); await auth.signOut(); }}>
