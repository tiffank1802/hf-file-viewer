...
| Table `favorites` vide alors que les favoris s’affichent dans l’app | les écritures sont coupées **avant** Appwrite (`VITE_APPWRITE_DATABASE_ID` vide dans le build) ou refusées par la table (`create()` absent, ou `users/verified` alors que le compte n’est pas encore vérifié) | `npm run appwrite:status` lit les `$permissions` réelles ; la ligne « Favoris : n · état … » du panneau de compte affiche désormais `permission refusée` avec la raison, au lieu de `table absente` |
